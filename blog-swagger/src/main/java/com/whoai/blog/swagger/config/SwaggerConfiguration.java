...
/**
 * Swagger2.x 版本和 3.x版本的配置存在一些差异
 * Swagger2.x {@link springfox.documentation.swagger2.annotations.EnableSwagger2 @EnableSwagger2} 启用 swagger
 * Swagger3.x {@link EnableOpenApi @EnableOpenApi} 注解可以不需要，通过变量控制是否启用 swagger
 * spring boot项目会自动扫描 springfox-boot-starter 依赖包下的spring.factories，自动配置相关组件
 */
@Configuration
